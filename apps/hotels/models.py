...
class RoomType(BaseModel):
    ...
    code = models.CharField(
        max_length=20,
        help_text="Short code (e.g., 'DLX', 'STD')"
    )
    ...
    images = models.JSONField(
        blank=True,
        null=True,
        help_text="Room photos with URLs"
    )

    # Status
    is_active = models.BooleanField(
        default=True,
        help_text="Available for booking"
    )
    ...
    def clean(self):
        ...
        if self.max_adults is not None and self.max_children is not None and self.max_occupancy is not None:
        ...
        if not self.bed_configuration or len(self.bed_configuration) == 0:
            raise ValidationError({
                'bed_configuration': 'At least one bed must be specified.'
            })
...
class Room(BaseModel):
    ...
    features = models.JSONField(
        blank=True,
        null=True,
        help_text="Room-specific features"
    )
    ...
    is_active = models.BooleanField(
        default=True,
        help_text="Available for assignment"
    )
