...
class HotelFactory(DjangoModelFactory):
    ...
    contact = factory.LazyFunction(
        lambda: {"phone": fake.phone_number(), "email": fake.company_email(), "website": fake.url()}
    )
    ...
    settings = factory.LazyFunction(
        lambda: {
            "booking_rules": {"min_stay": 1, "max_stay": 30},
            "policies": {"pets_allowed": fake.boolean(), "smoking_allowed": False},
        }
    )
...
class RoomTypeFactory(DjangoModelFactory):
    ...
    base_price = factory.Faker(
        "pydecimal", left_digits=3, right_digits=2, min_value=50, max_value=500
    )
    size_sqm = factory.Faker("pydecimal", left_digits=2, right_digits=2, min_value=20, max_value=80)

    bed_configuration = factory.LazyFunction(lambda: [{"type": "queen", "count": 1}])
